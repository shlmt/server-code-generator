        ...
        public override bool Generate()
        {
            ...
            foreach (Model model in Models)
            {
                ...
                routesCode += $"// {model.Name}\r\n"+
                    ...
                    ");\r\n" +
                    "\r\n" +
                    $"app.MapGet(\"/item/{{id}}\", async ({ProjectInfo.ProjectName}DbContext dbContext, int id) =>  {{\r\n" +
                    $"\tvar {lowerName} = await dbContext.{upperName}s.FindAsync(id);\r\n" +
                    $"\tif ({lowerName} == null) return Results.NotFound($\"{lowerName} with id {{id}} not found.\");\r\n" +
            ...
            string serverCode = "// this file generated by shlmt/ServerGenerator\r\n" +
                ...
                ? "\toptions.UseMySql(builder.Configuration.GetConnectionString(\"db\"), Microsoft.EntityFrameworkCore.ServerVersion.Parse(\"8.0.40-mysql\")));"
            ...
            try
            {
                ...
                string dbPackage = Database == Database.MySql ? "Pomelo.EntityFrameworkCore.MySql": "Microsoft.EntityFrameworkCore.SqlServer";
                ...
                Utils.RunCommand("dotnet", $"add package {dbPackage} --version 8.0.0", directoryPath);
                Utils.RunCommand("dotnet", "tool install --global dotnet-ef", directoryPath);

                if(!Utils.RunCommand("dotnet", $"ef dbcontext scaffold \"{DataBaseURL}\" {dbPackage} -f -c {ProjectInfo.ProjectName}DbContext", directoryPath))
                {
                    Console.WriteLine("models not created. can't countinue");
                    return false;
                }
